...
class RTTDataset(Dataset):
    ...
    def _precompute_switch_feat_global(self, sigma_sec=0.8, eps=1e-6):
        ...
        rel_t = (ts - ts[0]).astype(np.float32)  # global relative time
        ...
        # ---- Bump: use convolution to approximate Gaussian superposition
        ...
        kernel = np.exp(-0.5 * (t / np.float32(sigma_sec)) ** 2).astype(np.float32)

        imp = np.zeros((len(ts),), dtype=np.float32)
        imp[sw_pos] = sw_strength

        bump = np.convolve(imp, kernel, mode="same").astype(np.float32)
        bump = np.clip(bump, 0.0, 5.0).astype(np.float32)

        # prev/next dist：perform searchsorted on the global rel_t
        sw_t = rel_t[sw_pos].astype(np.float32)
        prev_idx = np.searchsorted(sw_t, rel_t, side="right") - 1
        next_idx = np.searchsorted(sw_t, rel_t, side="left")

        prev_dist = np.ones((len(ts),), dtype=np.float32)
        next_dist = np.ones((len(ts),), dtype=np.float32)
        ...
        has_next = next_idx < sw_t.shape[0]
        next_dt = np.zeros((len(ts),), dtype=np.float32)
        next_dt[has_next] = sw_t[next_idx[has_next]] - rel_t[has_next]
        next_dist[has_next] = np.clip(next_dt[has_next] / np.float32(self.window_size_sec), 0.0, 1.0)

        return np.stack([bump, prev_dist, next_dist], axis=-1).astype(np.float32)
...
class ImprovedAttentionFusionRTTTransformer(nn.Module):
    def __init__(self, input_dim, d_model, nhead, num_layers, num_steps_ahead,
        ...
        self.switch_proj = nn.Linear(3, d_model, bias=False)
    ...
    def forward(self, src, pos_encoding, switch_flag, window_mask,
        ...
        bottom_last = bottom_output[:, -1:, :]  # (B, 1, D)
...
def main_improved_attention_model(filenames, seq_len, num_steps_ahead, d_model):
    ...
    for filename in filenames:
        ...
        # use 1/2 data for training to speed up
        use_ratio = 0.5
        N_aligned = len(rtt_values)
        N_keep = int(N_aligned * use_ratio)

        # Ensure that at least one sliding window sample can still be formed
        min_need = seq_len + num_steps_ahead + 1
        if N_keep < min_need:
            N_keep = min(min_need, N_aligned)

        print(f"[TRIM] keep first {N_keep}/{N_aligned} points after alignment (ratio={use_ratio:.2f})")

        timestamps = timestamps[:N_keep]
        rtt_values = rtt_values[:N_keep]
        if sat_feat_full is not None:
            sat_feat_full = sat_feat_full[:N_keep]
            sat_valid_full = sat_valid_full[:N_keep]
        ...
        model_save_path = os.path.join(
            model_dir, f"attention_{base_filename}_ahead{num_steps_ahead}_L{seq_len}_D{d_model}_{run_tag}.pth"
        )
        ...
        for epoch in range(num_epochs):
            ...
            for step, (X_batch, pos_batch, sw_batch, far_bucket_batch, far_pos_batch,
                       window_mask_batch, y_rtt_batch, timestamps_y_batch) in enumerate(train_loader):
                ...
                loss = criterion(outputs_rtt, y_rtt_batch)
            ...
            with torch.no_grad():
                for (X_batch, pos_batch, sw_batch, far_bucket_batch, far_pos_batch,
                     window_mask_batch, y_rtt_batch, timestamps_y_batch) in val_loader:
                    X_batch = X_batch.to(device, non_blocking=True)
                    pos_batch = pos_batch.to(device, non_blocking=True)
                    sw_batch = sw_batch.to(device, non_blocking=True)
                    far_bucket_batch = far_bucket_batch.to(device, non_blocking=True)
        ...
        with torch.no_grad():
            for (X_batch, pos_batch, sw_batch, far_bucket_batch, far_pos_batch,
                 window_mask_batch, y_rtt_batch, timestamps_y_batch) in test_loader:
                X_batch = X_batch.to(device, non_blocking=True)
                pos_batch = pos_batch.to(device, non_blocking=True)
                sw_batch = sw_batch.to(device, non_blocking=True)
                far_bucket_batch = far_bucket_batch.to(device, non_blocking=True)
